...
Migrations V010–V012 add versioned knowledge and source evidence, independent human review, immutable publication, workspace/source access checks and contributor tracking. V014 adds human-managed source uploads, immutable original file bytes, extracted text, effective dates, classification, ownership and explicit source access. See [the full SOP walkthrough](../docs/sop-workflow.md) and [source upload guide](../docs/source-uploads.md). To run the focused integration suite, use `python -m pytest tests/test_sop_workflow.py tests/test_source_uploads.py tests/test_web.py -q` with the same isolated `KC_TEST_ADMIN_DSN`.
Migrations V015–V016 add Source Library permission management, restrict it to human workspace editors with source access, and grant the named source owner access at creation. Source access changes are audited and apply to all versions of a document.

Migrations V017–V018 add organization administrator grants and one-time user invitations. The [onboarding guide](../docs/organization-onboarding.md) covers first-organization provisioning, invitation creation, verified Auth0 acceptance and revocation.
