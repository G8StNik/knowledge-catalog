-- Human-managed source documents remain versioned evidence with explicit ACLs.
ALTER TABLE source.source_artifact
  ADD COLUMN owning_workspace_id uuid,
  ADD COLUMN owner_principal_id uuid,
  ADD COLUMN configuration_revision_id uuid,
  ADD COLUMN classification_id uuid,
  ADD COLUMN title text;

UPDATE source.source_artifact SET title=external_key WHERE title IS NULL;
ALTER TABLE source.source_artifact ALTER COLUMN title SET NOT NULL;
ALTER TABLE source.source_artifact ADD CHECK (btrim(title)<>'');
ALTER TABLE source.source_artifact ADD FOREIGN KEY (organization_id,owning_workspace_id)
  REFERENCES core.workspace(organization_id,workspace_id) ON DELETE RESTRICT;
ALTER TABLE source.source_artifact ADD FOREIGN KEY (organization_id,owner_principal_id)
  REFERENCES identity.principal(organization_id,principal_id) ON DELETE RESTRICT;
ALTER TABLE source.source_artifact ADD FOREIGN KEY (organization_id,configuration_revision_id,classification_id)
  REFERENCES governance.classification(organization_id,configuration_revision_id,classification_id) ON DELETE RESTRICT;
ALTER TABLE source.source_artifact ADD CHECK ((configuration_revision_id IS NULL)=(classification_id IS NULL));

ALTER TABLE source.artifact_version
  ADD COLUMN file_name text,
  ADD COLUMN media_type text,
  ADD COLUMN original_content bytea,
  ADD COLUMN original_content_hash text GENERATED ALWAYS AS
    (CASE WHEN original_content IS NULL THEN NULL ELSE encode(public.digest(original_content,'sha256'),'hex') END) STORED,
  ADD COLUMN uploaded_by uuid,
  ADD COLUMN effective_from date,
  ADD COLUMN effective_to date;
ALTER TABLE source.artifact_version ADD CHECK (file_name IS NULL OR btrim(file_name)<>'');
ALTER TABLE source.artifact_version ADD CHECK (octet_length(original_content)<=10485760);
ALTER TABLE source.artifact_version ADD CHECK (effective_to IS NULL OR effective_from IS NULL OR effective_to>=effective_from);
ALTER TABLE source.artifact_version ADD FOREIGN KEY (organization_id,uploaded_by)
  REFERENCES identity.principal(organization_id,principal_id) ON DELETE RESTRICT;

CREATE FUNCTION source.document_command(action text,payload jsonb,event_id uuid) RETURNS uuid
...
BEGIN
  ...
  IF nullif(payload->>'content_base64','') IS NULL OR nullif(btrim(payload->>'text_content'),'') IS NULL
      ...
      OR payload->>'media_type' NOT IN ('application/pdf','text/plain','text/markdown') THEN
